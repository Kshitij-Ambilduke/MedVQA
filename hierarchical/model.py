...
class VisualFeatures(nn.Module):
...
class CoAttention(nn.Module):
    def __init__(self, num_embeddings, num_classes, embed_dim=128, k=64):
        super().__init__()
    ...
    def forward(self, image, question):                   
        ...
        words = words.permute(0, 2, 1)  # Words: B x 512 x L
        ...
        words = words.permute(0, 2, 1)                                      # Words: B x L x 512
    ...
    def parallel_co_attention(self, V, Q):  # V : B x 512 x 196, Q : B x L x 512
        ...
        a_v = F.softmax(torch.matmul(torch.t(self.w_hv), H_v), dim=2) # B x 1 x 196
        a_q = F.softmax(torch.matmul(torch.t(self.w_hq), H_q), dim=2) # B x 1 x L
...
class Identity(nn.Module):
    def __init__(self, coattention, visualfeatures, vocab_len):
        super().__init__()
        self.vocab_len = vocab_len          #(num_embeddings, num_classes, embed_dim=512, k=128)
        self.coattention = coattention 
        self.visualfeatures = visualfeatures 

    def forward(self, images, questions):
        image_features = self.visualfeatures(images)
        out = self.coattention(image_features, questions)
        return out 
